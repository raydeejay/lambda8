(do
 ...
 (= cadr (fn (x) (car (cdr x))))
  ...
  ;; display a nice tree in the console as files are loaded
  
  ;; fbl - file being loaded
  ;; fbli - file being loaded indent
  (= dofile
     (fn (filename)
         (let (old-fbl fbl old-fbli fbli)
           (= fbl filename fbli (+ 2 (or fbli -2)))
           (times fbli (fn () (prin " ")))
           (print "Loading" fbl)
           (let (result (dostring (loads filename) filename))
             (= fbl old-fbl fbli old-fbli)
             result))))
