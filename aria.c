...
#define NUM_COMPARE_FUNC(NAME, OP)                      \
  static ar_Value *NAME(ar_State *S, ar_Value *args) {  \
    double left = ar_check_number(S, ar_car(args));     \
    int res = 1;                                        \
    while ( (args = ar_cdr(args)) && res ) {            \
      res = left OP ar_check_number(S, ar_car(args));   \
      left = ar_check_number(S, ar_car(args));          \
    }                                                   \
    return res ? S->t : NULL;                           \
  }
...
static ar_Value *f_lsh(ar_State *S, ar_Value *args) {
  int a = (int) ar_check_number(S, ar_car(args));
  int b = (int) ar_check_number(S, ar_nth(args, 1));
  return ar_new_number(S, a << b);
}

static ar_Value *f_rsh(ar_State *S, ar_Value *args) {
  int a = (int) ar_check_number(S, ar_car(args));
  int b = (int) ar_check_number(S, ar_nth(args, 1));
  return ar_new_number(S, a >> b);
}

static ar_Value *f_bitand(ar_State *S, ar_Value *args) {
  int a = (int) ar_check_number(S, ar_car(args));
  int b = (int) ar_check_number(S, ar_nth(args, 1));
  return ar_new_number(S, a & b);
}

static ar_Value *f_bitor(ar_State *S, ar_Value *args) {
  int a = (int) ar_check_number(S, ar_car(args));
  int b = (int) ar_check_number(S, ar_nth(args, 1));
  return ar_new_number(S, a | b);
}
